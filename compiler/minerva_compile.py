#!/usr/bin/env python3
"""
minerva_compile.py v1.3.1-Athena
...
class Compiler:
    ...
    def compile(self):
        ...
        # S = structure || nonce(IV) || weight_count || bias_count — mirrors
        # mnv_struct_auth.h exactly (same values emitted into the crypto header).
        arch_id=ARCH_BNN if self.quant=='binary' else ARCH_MLP
        wcount=sum(o[1] for o in offsets); bcount=sum(o[2] for o in offsets)
        S=(struct_preamble(arch_id,len(self.model.layers),layers=self.model.layers)
           +nonce+_le32(wcount)+_le32(bcount))
    ...
    def _emit_c(self,ct,nonce,mac,offsets):
        ...
        L=['/**',' * weights.c — AUTO-GENERATED by minerva_compile.py v1.3.1',
           f' * Bias quant: {mode}','*/',
    ...
    def _emit_dims(self):
        """Pure model-topology macros — NO includes, every macro self-guarded.

        mnv_config.h pulls this in via __has_include so the engine and the
        application are built with identical buffer sizes. Keeping it free of
        includes lets it be adopted before anything else in the config."""
        m=self.model
        L=['/** mnv_model_dims.h — AUTO-GENERATED by minerva_compile.py v1.3.1',
           ' *  Model topology ONLY. No includes. Auto-included by mnv_config.h',
           ' *  (via __has_include) so every translation unit agrees on the',
           ' *  compile-time buffer sizes. Command-line -D overrides still win. */',
           '#ifndef MNV_MODEL_DIMS_H','#define MNV_MODEL_DIMS_H','',
           '#ifndef MNV_INPUT_SIZE',  f'#define MNV_INPUT_SIZE    {m.input_size}U',  '#endif',
           '#ifndef MNV_NUM_LAYERS',  f'#define MNV_NUM_LAYERS    {len(m.layers)}U', '#endif']
        for i,l in enumerate(m.layers):
            L+=[f'#ifndef MNV_LAYER_{i}_SIZE', f'#define MNV_LAYER_{i}_SIZE  {l.out_size}U', '#endif']
        L+=['#ifndef MNV_OUTPUT_SIZE', f'#define MNV_OUTPUT_SIZE   {m.output_size}U', '#endif',
            '','#endif /* MNV_MODEL_DIMS_H */']
        return '\n'.join(L)+'\n'

    def _emit_h(self,ct,offsets):
        m=self.model
        # Topology macros live in mnv_model_dims.h (single source of truth,
        # also adopted by the engine via mnv_config.h). Include it first so
        # this header is self-contained for the application TU too.
        L=['/** weights.h — AUTO-GENERATED by minerva_compile.py v1.3.1 */',
           '#ifndef MNV_WEIGHTS_H','#define MNV_WEIGHTS_H',
           '#include "mnv_model_dims.h"','#include "minerva.h"','',
           f'#ifndef MNV_ENCRYPTED_LEN',f'#define MNV_ENCRYPTED_LEN {len(ct)}U','#endif','',
...
# ── 1D-CNN compiler ─────────────────────────────────────────────────────────────
def _q8_arr(arr):
    """Quantize to int8 with a symmetric per-tensor scale; return np.int8 array."""
    flat=arr.flatten().astype(np.float32)
    s=float(np.max(np.abs(flat)))
    if s==0: return np.zeros(len(flat),dtype=np.int8)
    return np.clip(np.round(flat/s*127),-128,127).astype(np.int8)

class CnnCompiler:
    """Compile a 1D-CNN (conv -> ReLU -> maxpool -> dense) to the exact blob
    the engine reads: [kernels F*K, filter-major][conv_bias F][dense_W.T
    OUT*FLAT, output-major][dense_bias OUT]. See mnv_cnn1d.c.

    npz schema:
      conv_w   float [F, K]        input_len  scalar int
      conv_b   float [F]           pool_size  scalar int
      dense_w  float [FLAT, OUT]   (standard dense; transposed here)
      dense_b  float [OUT]
    """
    def __init__(self,data,key,target):
        self.key=key; self.target=target
        self.progmem='avr' in target.lower() or 'tiny' in target.lower() or 'mega' in target.lower()
        self.conv_w =data['conv_w'].astype(np.float32)
        self.conv_b =data['conv_b'].astype(np.float32)
        self.dense_w=data['dense_w'].astype(np.float32)
        self.dense_b=data['dense_b'].astype(np.float32)
        self.input_len=int(data['input_len']); self.pool=int(data['pool_size'])
        self.F,self.K=self.conv_w.shape
    ...
    def compile(self):
        kq =_q8_arr(self.conv_w)          # [F*K] filter-major (row-major [F,K])
        cbq=_q8_arr(self.conv_b)          # [F]
        dwq=_q8_arr(self.dense_w.T)       # [OUT*FLAT] output-major (transpose)
        ...
        # S = structure || nonce(IV) || weight_count || bias_count.
        ...
        debug={'conv_w_q':kq.reshape(self.F,self.K),'conv_b_q':cbq,
               ...
               'meta':np.array([self.input_len,self.OUT,self.K,self.F,self.pool,
                                self.pool_len,self.FLAT,self.shift],dtype=np.int64)}
        return self._emit_c(ct,nonce,mac,wcount,bcount),self._emit_h(len(ct)),self._emit_dims(),debug

    def _emit_dims(self):
        L=['/** mnv_model_dims.h — AUTO-GENERATED by minerva_compile.py v1.3.1 (CNN1D)',
           ' *  Model topology ONLY. No includes. Auto-included by mnv_config.h. */',
           '#ifndef MNV_MODEL_DIMS_H','#define MNV_MODEL_DIMS_H','',
           '#ifndef MNV_INPUT_SIZE',      f'#define MNV_INPUT_SIZE       {self.input_len}U','#endif',
           '#ifndef MNV_OUTPUT_SIZE',     f'#define MNV_OUTPUT_SIZE      {self.OUT}U','#endif',
           '#ifndef MNV_CNN_KERNEL_SIZE', f'#define MNV_CNN_KERNEL_SIZE  {self.K}U','#endif',
           '#ifndef MNV_CNN_NUM_FILTERS', f'#define MNV_CNN_NUM_FILTERS  {self.F}U','#endif',
           '#ifndef MNV_CNN_POOL_SIZE',   f'#define MNV_CNN_POOL_SIZE    {self.pool}U','#endif',
           '#ifndef MNV_CNN_DENSE_SHIFT', f'#define MNV_CNN_DENSE_SHIFT  {self.shift}U','#endif',
           '','#endif /* MNV_MODEL_DIMS_H */']
        return '\n'.join(L)+'\n'

    def _emit_h(self,ctlen):
        L=['/** weights.h — AUTO-GENERATED by minerva_compile.py v1.3.1 (CNN1D) */',
           '#ifndef MNV_WEIGHTS_H','#define MNV_WEIGHTS_H',
           '#include "mnv_model_dims.h"','#include "minerva.h"','',
           f'#ifndef MNV_ENCRYPTED_LEN',f'#define MNV_ENCRYPTED_LEN {ctlen}U','#endif','',
           'extern const mnv_model_t         mnv_model;',
           'extern const mnv_crypto_header_t mnv_crypto_hdr;',
           'extern const uint8_t             mnv_encrypted_weights[];','',
           '#endif /* MNV_WEIGHTS_H */']
    ...
    def _emit_c(self,ct,nonce,mac,wcount,bcount):
        pm='PROGMEM' if self.progmem else ''
        L=['/**',' * weights.c — AUTO-GENERATED by minerva_compile.py v1.3.1 (CNN1D)',
           ' * The CNN1D engine derives its shape from the MNV_CNN_* macros and',
           ' * ignores the per-layer descriptor array (num_layers = 0).','*/','',
           '#include "weights.h"','#include "minerva.h"','#include "secrets.h"']
        if self.progmem: L.append('#include <avr/pgmspace.h>')
        L+=['',self._ca('mnv_encrypted_weights',ct),'']
        # Crypto header stays in RAM (no PROGMEM): the engine reads
        ...
        L+=[f'const mnv_crypto_header_t mnv_crypto_hdr = {{',
            ...
            f'    .bias_count   = {bcount}U,','};','']
        L+=['const mnv_model_t mnv_model = {',
            '    .version           = MNV_ABI_VERSION,',
            '    .num_layers        = 0U,',
            '    .layers            = NULL,',
            '    .crypto            = &mnv_crypto_hdr,',
...
# ── CLI ───────────────────────────────────────────────────────────────────────
def main():
    p=argparse.ArgumentParser(description='MINERVA Compiler v1.3.1-Athena')
    ...
    # 1D-CNN models (conv_w present) take a dedicated path — different blob
    # layout and a descriptor-less model (num_layers = 0).
    ...
    if 'conv_w' in _raw:
        ...
        if args.quant!='q8':
            print(f'[minerva] NOTE: CNN1D compilation is Q8 only; ignoring --quant {args.quant}')
        c_src,h_src,dims_src,debug=cc.compile()
        out=Path(args.out_dir); out.mkdir(parents=True,exist_ok=True)
        (out/'weights.c').write_text(c_src)
        (out/'weights.h').write_text(h_src)
        (out/'mnv_model_dims.h').write_text(dims_src)
    ...
    c_src,h_src,dims_src,debug=compiler.compile()
    ...
    print(f'          weights.c + weights.h + mnv_model_dims.h -> {out}')
    print(f'          Build EVERY .c (engine + app) with -I{out} so mnv_config.h')
    print(f'          adopts the model topology (no need to edit mnv_config.h).')
