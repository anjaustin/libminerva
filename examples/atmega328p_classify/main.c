...
#include "minerva.h"
#include "mnv_hal.h"     /* mnv_hal_fatal() — watchdog reset on unrecoverable error */
...
int main(void)
{
    ...
    while (1) {
        ...
        switch (status) {
            ...
            case MNV_ERR_TAMPER:
            case MNV_ERR_GLITCH:
                /* Unrecoverable: signal, then watchdog-reset rather than
                 * continue in an undefined state (threat model §4.4). */
                led_error_blink(3);
                mnv_hal_fatal();   /* enables WDT, spins until reset — no return */
                break;
